...
import { FileText, Download, Eye, Code, Lightbulb, Users, Target, Calendar, DollarSign, Home } from "lucide-react";
...
export const PRDDisplay = ({ productData, clarificationResponses, onBack, onHome }: PRDDisplayProps) => {
  const [activeView, setActiveView] = useState("overview");

  const handleDownload = () => {
    const prdContent = generatePRDContent();
    const blob = new Blob([prdContent], { type: 'text/markdown' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${productData.productName.replace(/\s+/g, '-').toLowerCase()}-prd.md`;
...
### Product Overview
**Mission:** ${mockPRDContent.overview.content.mission}

**Vision:** ${mockPRDContent.overview.content.vision}
...
**Business Goals:** ${productData.businessGoals || "Not specified"}

### Key Objectives
${mockPRDContent.overview.content.objectives.map(obj => `- ${obj}`).join('\n')}

### User Stories

${mockPRDContent.userStories.map(epic => `
#### ${epic.epic}
${epic.stories.map(story => `- ${story}`).join('\n')}
`).join('\n')}

### Technical Specifications
- **Architecture:** ${mockPRDContent.technicalSpecs.architecture}
- **Database:** ${mockPRDContent.technicalSpecs.database}
- **Hosting:** ${mockPRDContent.technicalSpecs.hosting}
- **Security:** ${mockPRDContent.technicalSpecs.security}
- **Performance:** ${mockPRDContent.technicalSpecs.performance}

### Development Timeline
${mockPRDContent.timeline.phases.map(phase => `- **${phase.phase}** (${phase.duration}) - ${phase.status}`).join('\n')}
...
---
*Generated by AI-driven SDLC Planning Tool*
`;
  };
